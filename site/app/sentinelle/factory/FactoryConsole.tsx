...
import { ArrowUp, ArrowUpRight, BriefcaseBusiness, Check, ChevronDown, Clock3, Film, Globe2, History, Link2, Loader2, LockKeyhole, Network, Newspaper, Plus, Send, Settings2, Sparkles, Upload, X } from "lucide-react";
...
type Mode = "clips" | "news" | "business" | "free";
type Clip = { id: string; accountId: string; hook: string; title: string; why: string; radarScore: number | null; durationSec: number | null; status: string; previewUrl: string; posterUrl: string; mediaSha256: string; postUrl: string; error: string; renderError: string; syntheticVoice?: boolean; sources?: { url: string; title: string; eventDate: string }[]; metrics?: { views: number; likes: number; comments: number; shares: number; observedAt: string }; publishId?: string; submittedAt?: string; tiktokStatus?: string; lastCheckedAt?: string; receipt?: { approvedAt: string; mediaSha256: string } };
type Run = { id: string; prompt: string; accountId: string; accountIds: string[]; status: string; mode?: Mode; needsInput?: string; answer: string; error: string; createdAt: string; clips: Clip[]; events: { at: string; message?: string; stage: string }[] };
...
const stages: Record<string, string> = { worker: "Le worker prépare la sélection", montage: "Montage en cours", ready: "La sélection est prête", publishing: "Envoi vers TikTok", processing: "TikTok traite les vidéos", attention: "Un envoi est à vérifier", needs_input: "Une précision est nécessaire", validating: "Vérification", done: "Suivi des publications", failed: "À reprendre", interrupted: "Traitement interrompu" };
...
export default function FactoryConsole() {
  ...
  const [editPreset, setEditPreset] = useState("natural");
  ...
  const upload = useRef<HTMLInputElement>(null), action = useRef(false), known = useRef(new Set<string>()), sourcesInitialized = useRef(false);
  const refresh = useCallback(async () => {
    ...
    setSources(sources); if (!sourcesInitialized.current) { sourcesInitialized.current = true; setSourceId((current) => current || sources.find((s) => /niel/i.test(s.name))?.id || ""); }
  ...
  const canPublish = !busy && online && live && selectedClips.length > 0 && chosen.length > 0 && chosen.every((a) => a.connected) && privacy.includes(settings.privacyLevel) && settings.musicUsageConfirmed && (!selectedClips.some((c) => c.syntheticVoice) || settings.isAigc) && (!settings.commercialDisclosure || settings.brandContent || settings.brandOrganic) && !(settings.brandContent && settings.privacyLevel === "SELF_ONLY");
  ...
  async function launch() {
    ...
    try { const data = await post("/runs", { prompt, count, mode, editPreset, accountIds: destinations, sourceId }); known.current = new Set(); setSelected([]); setTitles({}); setSettings({ ...defaults }); setRun(data.run); }
    ...
    <div className={styles.workspace}>
      ...
      <main className={styles.main}>
        {auth === "locked" ? <section className={styles.access}><LockKeyhole size={28} /><h1>Ton studio privé.</h1><form onSubmit={async (e) => { e.preventDefault(); try { await post("/session", { key }); setError(""); await refresh(); } catch (err) { setError((err as Error).message); } }}><label htmlFor="access">Code d’accès</label><input id="access" type="password" value={key} onChange={(e) => setKey(e.target.value)} autoComplete="off" /><button className={styles.primary}>Ouvrir le studio <ArrowUpRight size={17} /></button></form></section> : <>
          <div className={styles.pageHead}><div><p className={styles.eyebrow}>ESPACE DE PRODUCTION</p><h1>{tab === "network" ? "Un studio. Tes comptes." : tab === "history" ? "Les dernières sessions." : "Qu’est-ce qu’on crée ?"}</h1></div><span className={styles.target}>{destinations.length === 1 ? `@${destinations[0]}` : `${destinations.length} comptes`}</span></div>
          {tab === "studio" && <>
            <div className={styles.modes} role="group" aria-label="Type de création">{([{ id: "clips", label: "Clips", icon: Film, prompt: "Choisis les meilleurs moments de Lucia, drôles et compréhensibles sans contexte." }, { id: "news", label: "Actualité", icon: Newspaper, prompt: "Prépare un point sur l’actualité d’hier en région parisienne, avec des sources datées et vérifiées." }, { id: "business", label: "Mon activité", icon: BriefcaseBusiness, prompt: "Prépare une vidéo pour présenter mon activité : " }, { id: "free", label: "Libre", icon: Sparkles, prompt: "" }] as const).map((item) => <button key={item.id} type="button" aria-pressed={mode === item.id} disabled={busy} onClick={() => { setMode(item.id); setSourceId(""); setPrompt(item.prompt); }}><item.icon size={17} /><span>{item.label}</span></button>)}</div>
            <form className={styles.composer} onSubmit={(e) => { e.preventDefault(); launch(); }}>
              ...
              <textarea id="prompt" rows={2} maxLength={4000} value={prompt} disabled={busy} onChange={(e) => setPrompt(e.target.value)} />
              <div className={styles.composerFoot}><div className={styles.sourceTools}><label className={styles.sourcePicker}><Film size={14} /><select aria-label="Source vidéo" value={sourceId} disabled={busy || uploading} onChange={(e) => setSourceId(e.target.value)}><option value="">{mode === "clips" ? "Bibliothèque Lucia" : "Recherche & script"}</option>{sources.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><ChevronDown size={12} /></label><button type="button" className={styles.iconButton} aria-label="Importer une vidéo" title="Importer une vidéo (80 Mo, 5 min maximum)" disabled={busy || uploading} onClick={() => upload.current?.click()}>{uploading ? <Loader2 size={18} className={styles.spin} /> : <Plus size={18} />}</button><input hidden ref={upload} type="file" accept="video/mp4,video/quicktime,video/webm" onChange={(e) => { if (e.target.files?.[0]) importVideo(e.target.files[0]); e.target.value = ""; }} /></div><button className={styles.primary} disabled={busy || uploading || auth !== "ready" || !online || !prompt.trim()}>{busy ? <Loader2 size={16} className={styles.spin} /> : <ArrowUp size={17} />}<span>{busy ? "En cours" : `Créer ${count} vidéo${count > 1 ? "s" : ""}`}</span></button></div>
              <div className={styles.productionOptions}><label>Quantité <select aria-label="Nombre de vidéos" value={count} disabled={busy} onChange={(e) => setCount(Number(e.target.value))}>{[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}</option>)}</select></label><label>Montage <select aria-label="Style de montage" value={editPreset} disabled={busy} onChange={(e) => setEditPreset(e.target.value)}><option value="natural">Naturel</option><option value="dynamic">Dynamique</option></select></label></div>
            </form>
            <div className={styles.editorialBar}><button onClick={() => setTab("network")} className={styles.textButton}><Globe2 size={14} />{destinations.length === 1 ? `@${destinations[0]}` : `${destinations.length} comptes`}<ChevronDown size={12} /></button><span>9:16 <b>·</b> {sourceId || mode === "clips" ? "Voix originale" : "Bulletin · voix de synthèse"}</span></div>
            <PublicationTracker clips={clips} />
            <section className={styles.production} aria-label="Sélection vidéo">
              ...
              {run && <div className={styles.progress} aria-live="polite">{busy || run.status === "processing" ? <Loader2 className={styles.spin} size={14} /> : <Check size={14} />}<span>{run.error || run.needsInput || run.events?.at(-1)?.message || run.answer}</span></div>}
              {clips.length ? <div className={styles.clips}>{clips.map((c, i) => <article className={styles.clip} key={c.id}>
                ...
                {c.metrics && <p className={styles.clipDetail} title={`API TikTok · ${new Date(c.metrics.observedAt).toLocaleString("fr-FR")}`}>{c.metrics.views} vues · {c.metrics.likes} likes · {c.metrics.shares} partages</p>}
                <details className={styles.clipDetail}><summary>Choix éditorial{c.radarScore != null ? ` · ${c.radarScore}/100` : ""}</summary><p>{c.why}</p>{c.syntheticVoice && <p>Bulletin original · voix de synthèse</p>}{c.sources?.map((s) => <p key={s.url}><a href={s.url} target="_blank" rel="noreferrer">{s.title} <ArrowUpRight size={12} /></a>{s.eventDate && ` · ${s.eventDate}`}</p>)}{c.receipt && <p>Validé le {new Date(c.receipt.approvedAt).toLocaleString("fr-FR")} · SHA256 {c.receipt.mediaSha256.slice(0, 12)}</p>}</details>
            ...
            {selectedClips.length > 0 && <section className={styles.publishZone} aria-label="Publication TikTok">
              {selectedClips.some((c) => c.syntheticVoice) && <p className={styles.disclosureNote}>Voix de synthèse : la mention « Contenu généré par IA » est requise pour cette sélection.</p>}
